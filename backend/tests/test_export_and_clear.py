...
def test_export_endpoints(session: Session):
    ...
    try:
        ...
        assert "Mfg_Part_Num" in headers or "SKU - MY_PART_NUMBER" in headers or "SKU" in headers
        assert "BRAND_NAME" in headers or "MANUFACTURER_NAME" in headers or "Brand" in headers
...
def test_clear_all_documents(session: Session):
    app.dependency_overrides[get_session] = lambda: session
    client = TestClient(app)
    try:
